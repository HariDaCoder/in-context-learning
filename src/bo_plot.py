        ...
        for record in document["records"]:
            ...
            if key not in groups:
                groups[key] = {"identity": dict(zip(
                    ("protocol_id", "model", "checkpoint_id", "training_seed", "d"), key)),
                    "protocol": document["metadata"]["protocol"], "cells": defaultdict(dict)}
            group = groups[key]
...
def summarize_group(group, target):
    ...
    for (rho, snr, k), records in sorted(cells.items()):
        ...
        summary.append({"rho": rho, "snr": _snr_json(snr), "k": k,
...
def summarize_training_replicates(groups, target):
    ...
    outputs = []
    for (protocol_id, model, d), replicas in sorted(buckets.items()):
        ...
        for rho, snr, k in all_cells:
            ...
            cell_summaries.append({"rho": rho, "snr": _snr_json(snr), "k": k, "metrics": metrics})
...
def plot_group(group, out_dir, metric="clean_query_mse", target=0.5, log_y=False):
    ...
    for axis_type in ("k", "effective_rank_over_d"):
        ...
        for ax, rho in zip(axes_grid[0], rhos):
            for index, snr in enumerate(snrs):
                ...
                for k in ks:
                    ...
                    x = k if axis_type == "k" else record.get(
            ...
            ax.set_xlabel("Context examples k" if axis_type == "k" else "Effective context ratio k_eff / d")
...
def main():
    ...
    print(f"Wrote {len(figures)} figures and {destination}")
